...
const solution = () => {
  ...
  const dijkstra = (start) => {
    ...
    while (!queue.isEmpty()) {
      ...
      if (distance[current][currentPave] < currentCost) continue;

      for (let i = 0; i < graph[current].length; i++) {
        const { node: next, cost: nextCost } = graph[current][i];
        const totalCost = currentCost + nextCost;

        if (totalCost < distance[next][currentPave]) {
          queue.insert({ node: next, cost: totalCost, pave: currentPave });
          distance[next][currentPave] = totalCost;
        }

        if (currentPave < K && currentCost < distance[next][currentPave + 1]) {
          queue.insert({
            node: next,
            cost: currentCost,
            pave: currentPave + 1,
          ...
          distance[next][currentPave + 1] = currentCost;
        }
      }
    }
  };
